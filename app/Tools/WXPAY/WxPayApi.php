...
use App\Tools\WXPAY\Lib\WxPayResults;
use App\Tools\WXPAY\Lib\WxPayReport;
...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		Log::info("WxPayApi::unifiedOrder".json_encode($response));
	...
	public static function micropay($inputObj, $timeOut = 10)
	{
		...
		Log::info("WxPayApi::micropay".json_encode($response));
	...
	public static function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		Log::info("WxApi::notify");
		try {
			$result = WxPayResults::Init($xml);
		} catch (WxPayException $e){
			$msg = $e->errorMessage();
			Log::info(" WxPayApi->notify : try catch false $msg");
			return false;
		}
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, base_path()."/config/weixin/apiclient_cert.pem");
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, base_path()."/config/weixin/apiclient_key.pem");
		}
		//post提交方式
