...
class McyAutoManController extends Controller
{
	...
	public function automanRun(Request $request)
	{
		...
		if ($type == 2)
		{
			...
			$products = McyProduct::where('is_delete',0)->where('category_id',$category_id)->get();
			foreach($products as $product)
			{
				$product->go_zhiding = $is_auto;
				$product->save();
			}
		...
		if ($product)
		{
			...
			if ($yungou_shop)
			{
				...
				// }else{
					...
					if ($yungou_number >= $yungou_shop->shenyurenshu)
					{
						$yungou_number = $yungou_shop->shenyurenshu;
						// 人数不足
						// if ($yungou_shop->shenyurenshu > 5)
						// {
						// 	$yungou_number= mt_rand(1,6);
	...
	{
		$url = url()->current()."/api/go_product_daojishi/".$yungou_id;
	...
	public function go_product_daojishi1(Request $request)
	{
		...
		if ($yungou_shop->status == 1)
		{
			...
			foreach($orders as $key => $order)
			{
				...
				if (($mcy_u->is_robot == 1) && ($s_w == 1))
				{
					if ($product->go_zhiding == 1){
						if ($yungou_shop->zhiding >= 2){
							...
							if ($key == 0){
										...
										WechatLog::info('测试码原始:'.$ceshi_ma);
										WechatLog::info('测试码次数原始:'.count($ma));
							...
							if ($key >= $t11) {
								...
								$ceshi_ma = $ma[$t];
	                            #$ceshi_ma = explode(",",$order->yungouma)[0];
								WechatLog::info('测试码修改：'.$ceshi_ma);
								WechatLog::info('测试码次数修改：'.count($ma));
								WechatLog::info('测试key：'.$key);
								$ceshi_order_id = $order->go_id;
								$ceshi_order_time = $order->created_at;
								$s_w = 0;
							}else{
			...
			if ($product->go_zhiding == 1 && $ceshi_id > 0)
			{
				$yungou_shop->zhiding = $ceshi_id;
				$huode_id = $ceshi_id;
				$huode_ma = $ceshi_ma;
			...
			if ($yungou_shop->zhiding == 0)
			{
				...
				if ($yungou_shop->zhiding == 1)
				{
					/* 测试 */
					/* 判断获奖人是否机器人 */
					WechatLog::info("正常开 ：".$huode_id.' 产品ID'.$product->product_id." 产品期数".$product->go_now_qishu." 中奖码：".$huode_ma);
					if ($mcy_user->is_robot == 0)
					{
						/* 非机器人，修改日期*/
						/* 找出购买记录中，有参与购买的测试账号 */
				...
				// 特定编号狸猫换太子
					...
					if ($t_order)
					{
                        ...
                        // $ceshi_ma = $ma[$w_t - 1];
                        $ceshi_ma = $ma[$t];
                        WechatLog::info('指定测试码'.$ceshi_ma);
                        WechatLog::info("指定测试 ：".$huode_id.' 产品ID'.$product->product_id." 产品期数".$product->go_now_qishu);
						...
						// 	// $yt->created_at = date("Y-m-d H:i:s",(strtotime($yt->created_at) - $cha));
							...
							$mcy_user = McyUser::where('is_delete',0)->where('mcy_user_id',$ceshi_id)->first();
						// }else{}
						WechatLog::info("测试开 ：".$huode_id.' 产品ID'.$product->product_id." 产品期数".$product->go_now_qishu." 中奖码：".$huode_ma);
					}else{}
				}
