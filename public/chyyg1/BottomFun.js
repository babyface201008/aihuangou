...
$(document).on("click", '.add[codeid]', function (o) {
    ...
    $.getJSON(Gobal.Webpath + '/mobile/cart/addShopCart/' + codeid + '/'+cartnum,{"qishu":qishu} ,function (data) {
        if (data.code == 1) {
            ...
            content = '<div class="cfm-title" style="color: #dc332d;font-size: 14px;font-weight: bold;">本期'+qishu+'已经更新到'+data.qishu+'期</div><p style="line-height: 30px;font-size: 12px;">是否继续购买?</p>';
            $.PageDialog.confirm(content, function () {
                $.getJSON(Gobal.Webpath + '/mobile/cart/addShopCart/' + codeid + '/'+cartnum,
                    function (data) {
                        if(data.code == 0){
                            eval(backcall_func)();
                            return;
