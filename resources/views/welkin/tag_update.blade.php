            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>修改标签<small><a href="/welkin/tag">返回标签列表</a></small></h5>
                                                ...
                                                <div class="smart-widget-body">
                                                    <form class="form-horizontal no-margin" id="article_create">
                                                        <div class="form-group">
                                                            ...
                                                            <div class="col-lg-9">
                                                                <input type="text"  value="{{@$tag->tag_name}}" class="form-control input-sm" placeholder="标签名称" datatype="*" name="tag_name" errmsg="标签名称不能为空" >
                                                            </div><!-- /.col -->
                                                        </div><!-- /form-group -->                              
                                                        <div class="text-right m-top-md">
                                                            <button type="button" class="btn btn-info btn-tag-update">修改</button>
                                                        </div>
                                                    </form>
                                                </div>
...
<script type="text/javascript">
    document.onkeydown = function(even){
        ...
        if (e && e.keyCode == 13)
        {
            $(".btn-tag-update").click();
        }
    }
    $(".btn-tag-update").on('click',function(e){
        e.preventDefault();
        var tag_name = $('input[name=tag_name]').val();
        $.ajax({
            type : 'post',
            url  : '/api/welkin/tag/update',
            dataType : 'json',
            data : {
                _token   : "{!! csrf_token() !!}",
                tag_id  : {{@$tag->tag_id}},
