            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>标签管理 <small> <a class="btn btn-sm btn-success" href="/welkin/tag/create">添加标签</a>
                        </small></h5>
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach($tags as $tag)
                                    <tr class="uid{{@$tag->tag_id}}">
                                 <!--  <td>
                                            <input type="checkbox" checked class="i-checks" name="input[]">
                                        </td> -->
                                        <td>{{@$tag->tag_name}}</td>
                                        <td>{{@$tag->nickname?$tag->nickname:$tag->username}}</td>
                                        <td>{{@$tag->created_at}}</td>
                                        <td>
                                        <a class="btn btn-sm btn-info updateuser" href="/welkin/tag/update?tag_id={{@$tag->tag_id}}">修改</a>
                                        <!-- <button type="button" class="btn btn-sm btn-danger deleteuser" uname="{{@$tag->tag_name}}" uid="{{@$tag->tag_id}}" >删除</button> -->
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            {{$tags->appends(['searchtext'=>@$searchtext,'starttime'=>@$_GET['starttime'],'endtime'=>@$_GET['endtime']])->links()}}
                        </div>
...
@section('my-js')
    ...
    $(".btnsearch").on('click',function(){
        ...
        location.href = '/welkin/tag?searchtext='+ searchtext + '&starttime=' + starttime + '&endtime='+ endtime;
    });

    $('.deleteuser').on('click',function(){
        var uid = $(this).attr('uid'),
            name = $(this).attr('uname');
        parent.layer.confirm('确定删除名称为' + name + '的标签?',function(){
           $.ajax({
            url : '/api/welkin/tag/delete',
              type : 'post',
              async : false,
              dataType : 'json',
              data : {
