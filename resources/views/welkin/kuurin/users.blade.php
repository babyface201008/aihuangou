            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>用户管理 <small> <a class="btn btn-sm btn-success" href="/welkin/kuurin/user/create">添加用户</a>
                        </small></h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr class="uid{{@$user->kuurin_user_id}}">
                                 <!--  <td>
                                        ...
                                        <td>
                                        <a class="btn btn-sm btn-info updateuser" href="/welkin/kuurin/user/update?user_id={{@$user->kuurin_user_id}}">修改</a>
                                        <button type="button" class="btn btn-sm btn-danger deleteuser" uname="{{@$user->username}}" uid="{{@$user->kuurin_user_id}}" >删除</button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            {{$users->appends(['searchtext'=>$searchtext,'starttime'=>@$_GET['starttime'],'endtime'=>@$_GET['endtime']])->links()}}
                        </div>
...
@section('my-js')
    ...
    $(".btnsearch").on('click',function(){
        ...
        location.href = '/welkin/kuurin/users?searchtext='+ searchtext + '&starttime=' + starttime + '&endtime='+ endtime;
    });
    ...
    $('.deleteuser').on('click',function(){
        ...
        parent.layer.confirm('确定删除名称为' + name + '的用户?',function(){
           $.ajax({
            url : '/api/welkin/kuurin/user/delete',
              type : 'post',
              async : false,
              dataType : 'json',
              data : {
